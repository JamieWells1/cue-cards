...
export function CueCard({ card, className }: CueCardProps) {
  ...
  const getTriggerGradient = (trigger: Trigger) => {
    const gradients = {
      ...
      start:
        "bg-gradient-to-br from-yellow-400 to-yellow-600 shadow-yellow-200",
    };
  ...
  return (
    <Card
      className={cn(
        "w-full max-w-2xl mx-auto animate-fade-in bg-white shadow-lg",
        className
      )}
    >
      <CardHeader className="pb-3 sm:pb-4 px-4 sm:px-6">
        {card.imageUrl && (
          <div className="flex justify-center mb-4">
            <img
              src={`/api/image-proxy?url=${encodeURIComponent(card.imageUrl)}`}
              alt={card.name}
              className="max-w-full max-h-56 object-contain rounded-lg shadow-md my-4"
            />
          </div>
        )}
      ...
      <CardContent className="space-y-4 sm:space-y-6 px-4 sm:px-6">
        ...
        <div className="bg-gray-50 rounded-lg p-3 sm:p-4 space-y-2 sm:space-y-3">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 sm:gap-2">
            <Text variant="muted" weight="bold" className="text-sm sm:text-lg">
              Album:
            </Text>
            <Text
              className={cn(
                getAlbumColor(card.album),
                "font-black text-base sm:text-xl"
              )}
            >
        ...
        {/* Abilities */}
        {(card.ability_triggers.length > 0 ||
          card.ability_descriptions.length > 0) && (
          <div className="space-y-3 sm:space-y-4">
            <Text
              size="lg"
              weight="bold"
              className="border-b-2 border-gray-200 pb-2 text-gray-900 text-lg sm:text-xl font-black"
            >
              Abilities
            </Text>
            <div className="space-y-3">
              {Array.from({
                length: Math.max(
                  card.ability_triggers.length,
                  card.ability_descriptions.length
                ),
              }).map((_, i) => (
                <div
                  key={i}
                ...
                >
                  ...
                  {card.ability_descriptions[i] && (
                    <Text
                      variant="default"
                      leading="relaxed"
                      className="text-sm sm:text-base text-gray-700 font-bold"
                    >
                      {card.ability_descriptions[i]}
                    </Text>
                  )}
